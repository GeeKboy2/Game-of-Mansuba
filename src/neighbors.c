...
unsigned int nombre_mouvements(struct world_t* world, unsigned int idx)
{
  ...
  compteur++; //Adding one case of multiple jumps.(if the multiple jump isn't possible the piece will do a simple jump if even a simple jump isn't possible it will stay in place.)
  return compteur;
}
